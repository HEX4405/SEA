...
void sys_yieldto(pcb_s* dest) 
{
	...
	__asm("mov r0, #5");
	__asm("swi #0");
	__asm("mov sp, %0" : : "r"(current_process->context.sp_user));
	__asm("mov lr, %0" : : "r"(current_process->context.lr_user));
	__asm("msr cpsr, %0" : : "r"(current_process->context.cpsr_user));
}
...
void __attribute__((naked)) swi_handler()
{
	...
	//On passe en mode system pour récupérer le lr_user et le sp_user
	__asm("cps #31");
	__asm("mov %0, sp" : "=r"(current_process->context.sp_user));
	__asm("mov %0, lr" : "=r"(current_process->context.lr_user));
	__asm("cps #19");
	
	//On récupère le spsr qui correspond au cpsr_user
	__asm("mrs %0, spsr" : "=r"(current_process->context.cpsr_user));
